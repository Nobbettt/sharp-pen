/* Static settings view. All dynamic strings are assigned through textContent/value. */
const vscode = acquireVsCodeApi();
const el = (id) => document.getElementById(id);
const els = {
  client: el("client"), theme: el("theme"), model: el("model"), models: el("models"),
  saveModel: el("save-model"), refreshModels: el("refresh-models"), modelProvider: el("model-provider"), manualModel: el("manual-model"), status: el("status"),
};
const label = { claude: "Claude Code", codex: "Codex", copilot: "GitHub Copilot", opencode: "OpenCode" };
const otherModel = "__sharp_pen_other_model__";
let state;
...
els.client.addEventListener("change", () => { showingManual = false; vscode.postMessage({ type: "setClient", client: els.client.value }); });
...
els.model.addEventListener("keydown", (event) => { if (event.key === "Enter") { event.preventDefault(); vscode.postMessage({ type: "setModel", model: els.model.value }); } });
