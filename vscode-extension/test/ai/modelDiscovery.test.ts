...
test("OpenCode model discovery stays unavailable with its adapter", async () => {
  await assert.rejects(discoverModels("opencode"), /unsupported/);
});

test("Codex app-server v0.155.1 model/list fixture uses its required handshake and page fields", () => {
  assert.deepEqual(codexInitializeParams, {
